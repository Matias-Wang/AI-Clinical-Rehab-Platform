...
## 🟢 20260228 最終進度存檔 (Snapshot)
- 主要作業檔案：20260228_Project_Rehab_Week1_Diagnostics.ipynb
- 診斷成果：
    * 一致性：確認 S1 與 S10 存在最大分佈偏移（$D \approx 0.15$）。
    * 物理稽核：抓到 S2、S8 的標籤雜訊與 S3 的硬體偏移。
    * 量化分析：雜訊比例極低（$< 0.2\%$），具備高度清洗價值。
    * 容量驗證：LOSO 切分模擬完成，各受試者數據量約在 500-550 個視窗，分佈穩定。
- 文件更新：已擬定 Spec/v3_1_multi_Technical_Spec.md 的 20260228 紀錄內容。

## 🟢 2026/03/08 研發紀錄：
1. 核心里程碑：跨受試者驗證 (LOSO) 
    - 實作日期：2026/03/07。
    - 動作：實作 10-Fold Leave-One-Subject-Out 訓練循環，取代原本隨機切分的驗證法。
    - 結果：初始平均準確率約 83.6%，確認 S2 (70.6%) 與 S8 (75.9%) 為性能瓶頸點。
2. 特徵工程迭代：重力向量分離案
    - 實作內容：在 schema.py 導入 Butterworth 低通濾波器（截止頻率 $0.3\ Hz$）提取重力分量 $Grav\_X, Y, Z$。
    - 技術轉折紀錄：
        * 修正 A：將 lfilter 升級為 filtfilt (零相位濾波)，解決因特徵相位延遲導致的 S1/S2 性能下降問題。
        * 修正 B：捨棄全特徵 $Z$-score，改採「選擇性標準化」搭配「全局比例縮放（$/10.0$）」。保留了重力的絕對物理方向，並對齊特徵量級。
3. 關鍵診斷發現
    - S2 Sitting悖論：儘管加入重力向量，S2 的坐姿仍有 98% 被判為彎腰。診斷圖顯示 S2 坐姿角度與群體彎腰角度高度重疊，純「角度特徵」不足以區分靜態與動態傾斜。