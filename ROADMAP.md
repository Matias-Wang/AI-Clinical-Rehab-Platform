...
| **目前進度** | 全面稽核 A 類 10 項與 B1／B3 完成；Stage 8 剩餘 B（推論阻塞，已評估為非急迫）與 E（壓力測試與交付） |
| **當前日期** | 2026-09-06 |
...
| 受試者群組 | 品質合格率 | 說明 |
|-----------|-----------|------|
| S01 - S02（低活動組） | 5.5% - 8.7% | 大量攔截為**正確行為** |
| S03 - S09 | 19.3% - 27.6% | 僅在動態執行區間通過 |
| S10（黃金標準） | 22.5% | 靜態區間自動攔截 |

「總視窗數」等於該受試者的預處理視窗數（S01=548、S10=525）。

> **基準已於 Stage 8 B1 變更**（2026-09-06）。舊基準為 S01=3.5%、S10=20.6%，
> 總視窗數約 1095——那是對「攤平重疊視窗」所得人工訊號重新切窗的結果，
> 每個樣本重複兩次且 50% 的視窗帶有訓練時未見過的 FFT 形態。
> 改為逐視窗評估後，數字才對應真實的預處理視窗。詳見 CHANGELOG [Stage 8 — B1/B3]。

> 品質合格率不受 Fix 3 影響（`ClinicalQualityGate` 只看 Grav_Y，跟 Acc/Mag Z-score 是分開的路徑）。Stage 8 B3 起，報表以「L7 召回率」取代原本無從判讀對錯的「AI 辨識 L7 次數」。
