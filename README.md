...
復健動作的療效取決於「做得對不對」，但現行監測依賴治療師人工觀察，無法即時、
客觀地量化。把感測器接上 AI 模型看似能解決，實際上會遇到兩層問題：

**第一層：訊號相似。** 靜態動作（Standing vs. Sitting）與動態動作
（Walking vs. Climbing Stairs）的時域特徵高度重疊，單看加速度難以區分。

**第二層（更棘手）：資料品質不均。** 本專案在診斷階段發現明顯的
**治療師集群效應（Therapist Clustering）**——S01–S04 群組在關鍵動作上的
$Grav\_Y$ 變異數極低（< 0.0003），推測是不同治療師的引導方式或感測器配戴
習慣，導致關鍵運動平面的訊號根本沒被記錄下來。

以高活動組（S05–S10）訓練的模型去預測低活動組時，準確率由 92% 崩跌至
**68.89%**。這不是模型不夠好，而是**輸入資料本身沒有足夠資訊**。

### 解決路線：Data-Centric 而非後端補償

我們先嘗試了「後端補償」——以 Rodrigues 旋轉公式校正配戴偏差。結果準確率
**再降 6.58%**（68.89% → 62.31%）。結論很明確：在極低訊噪比下，數學補償只會
放大隨機噪音。

因此改採 **Data-Centric AI** 路線，核心是一道**臨床品質閘門
（Clinical Quality Gate）**：

> 動作幅度不足時，系統**不預測**，直接亮紅燈告訴患者「動作幅度不夠，請加大」。

這是本專案最重要的設計決策——寧可不輸出，也不輸出基於無效資料的結果。對醫材
而言，一個沉默的系統遠優於一個自信地給出錯誤評分的系統。
...
早期版本以點對點的歐幾里德距離比對姿態，導致患者只要節奏比範本慢 0.05 秒，
分數就暴跌並誤觸黃燈。但**節奏慢不等於做錯**——懲罰速度差異與復健生物回饋的
目的相悖。改用 DTW（Sakoe-Chiba band, radius=16）後，系統只評估動作軌跡是否
正確，容忍時間軸上的伸縮。
...
專案的缺陷有共同模式：**文件寫著一套、程式碼跑著另一套，而且沒有東西會發現**。
因此建立了五層自動化閘門（`python verify.py --full`）：

| 層 | 內容 |
|----|------|
| 單元與回歸測試 | 髒數據防護、緩衝區復原、DTW 性質、預處理修正 |
| 規範與一致性 | CLAUDE.md 規範、**文件漂移偵測**、**notebook 相容性守門** |
| 模型準確率基準 | 對照訓練 notebook 的 S02/S07/S09/S10 準確率 |
| 驗收輸出平價 | `main.py` 輸出與 golden 基準逐行比對 |
| 效能煙霧測試 | 推論延遲與串流速率門檻 |

其中「文件漂移偵測」會解析 SPEC.md 的 API 表格，斷言記載的每個符號都真實
存在於程式碼中——這道檢查是因為文件曾長期記載三個不存在的方法而建立的。
...
- **自動化回歸測試**：`tests/` 提供 118 項測試，釘死 Fix 1/2/3 等致命修正與推論路徑選擇，並以突變測試驗證測試本身的鑑別力。
