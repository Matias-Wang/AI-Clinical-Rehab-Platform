# 研發歷程紀錄 — AI 臨床復健生物回饋平台

本文件依時間順序記錄本專案自 2026/02 起的技術探索、決策邏輯與實驗結果。

與 [CHANGELOG.md](CHANGELOG.md) 的分工：CHANGELOG 記錄「**改了什麼**」，
本文件記錄「**為什麼這樣改、依據是什麼、試過哪些失敗的路**」。

## 這份文件怎麼讀

全文橫跨 2026/02–09 共 19 則紀錄。若時間有限，以下三則最能代表本專案的技術判斷：

| 條目 | 為什麼值得看 |
|------|-------------|
| **2026/07/12 — Fix 3：訓練與推論一致性重大修復** | 最隱蔽的一次缺陷。重點不在修法，而在「如何被發現」——修正後表面症狀消失、看起來像修好了，實際準確率反而更糟，唯一能揭穿的是對照訓練 notebook 的基準準確率 |
| **2026/04/04 — 從演算法補償轉向臨床品質控制** | 本專案最重要的架構決策。先嘗試以 Rodrigues 旋轉做後端補償並失敗（-6.58%），才確立 Data-Centric 路線與「動作品質不足時拒絕預測」的臨床品質閘門 |
| **2026/09/06 — 串流重建缺陷** | 語法完全正確、執行也不會出錯的缺陷。是量測資料的物理性質（長度膨脹倍率、接縫跳變幅度、特徵形態分布）才暴露出來的 |

各條目與版本的對應關係，另見 [ARCHITECTURE.md](ARCHITECTURE.md) 的「技術世代演進」表。

---

## 品質機制與 SaMD 概念對應

本專案為個人作品集，非受法規監管的產品。但建置過程中導入的工程機制，
其設計意圖與 SaMD（Software as a Medical Device）框架的若干核心概念相對應：

| 本專案的機制 | 對應概念 |
|-------------|---------|
| `ClinicalQualityGate`：動作幅度不足或訊號含 NaN/Inf 時**拒絕輸出**，而非給出低分 | 風險控制措施（fail-safe）——降低「系統對不可信輸入給出臨床建議」的風險 |
| `verify.py` 五層閘門，每次修改後執行 | 設計驗證（Design Verification）——變更後確認輸出仍符合既定規格 |
| `tests/golden/main_acceptance.txt` 驗收基準逐行比對 | 迴歸驗證與變更管制——行為變更無法默默發生，必須明確更新基準並記錄原因 |
| `test_doc_consistency.py`：斷言 SPEC.md 記載的每個 API 都真實存在於程式碼 | 可追溯性（Traceability）——規格文件與實作不得脫節 |
| `requirements.txt` 全數鎖定版本 | 第三方軟體（SOUP）的版本控制與可重現性 |
| 突變測試：注入已修復的缺陷，確認測試如實失敗 | 驗證方法本身的有效性——避免「通過但無鑑別力」的測試 |

> 上述為概念對應，非合規聲明。實際 SaMD 開發尚需完整的品質管理系統、
> 風險管理檔案（ISO 14971）與臨床評估，本專案未涵蓋。

---

## 初始技術存檔（Generation 1，2026/02）

> 以下第 1–6 節為專案起始時的規格快照，保留原貌以呈現演進起點。
> **當前規格請參閱 [SPEC.md](SPEC.md)** ——
> 特徵維度已由 4 維擴展至 8 維混合域，驗證協議已改採 LOSO 交叉驗證。
...
- 輸入維度 (Input Shape)：$128 \times 4$（時序長度 128，特徵維度 4：加速規三軸 + 總加速度）。
  *（Generation 1 規格；現行為 128 × 8 混合域特徵，見 SPEC.md §2.2）*
