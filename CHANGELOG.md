...
## [Stage 8 — B1/B3] — 2026-09-06 — 串流重建缺陷修正（⚠️ 驗收基準變更）

### Fixed
- **B1 串流重建缺陷**：`main.py` 與 `ui_bridge.py` 原以 `X.reshape(-1, 8)` 將已 50% 重疊的視窗攤平當成「模擬即時串流」。實測（S01）顯示三個問題：
  1. 串流長度 70,144 步，為真實訊號 35,136 步的 **2.00 倍**——每個樣本重複出現兩次
  2. 每 128 步在時間軸倒退 64 步，接縫處的 Grav_Y 跳變達視窗內部的 **53 倍**
  3. 引擎重新切窗後，**50% 的視窗橫跨兩個原始視窗**，使第 8 維 FFT 能量出現兩個不同值——而該特徵在訓練資料中永遠是整個視窗的單一常數。這是與 Fix 3 同類的 train/serving skew

  FFT 為視窗級特徵且必須以原始尺度的 Magnitude 計算（見 Fix 3），在已標準化的逐幀串流中無法重建，因此改為**逐視窗評估**——唯一能餵入正確輸入形態的作法。

- **B3 驗收報表無從判讀**：`main.py` 載入 `y_test` 卻從不使用，「AI 辨識 L7 次數」是模型預測 Label 7 的原始次數，無法判斷正確與否（S08 顯示 17 次為全體最高，但無從得知是否誤判）。改為以真實標籤計算 **L7 召回率**，並顯示 L7 視窗數。無 L7 視窗通過閘門時顯示「—」，避免與「全部辨識錯誤」混淆。

### Added
- **`RealTimeBiofeedbackEngine.evaluate_window()`**：自 `process_live_frame()` 抽出的臨床決策流程，供串流路徑與批量／重播路徑共用，確保兩者判定邏輯完全一致。

### Changed
- **`ui_bridge.stream_subject()`**：改為逐視窗推播，依真實時間節奏（相鄰視窗相距 stride 幀，`--realtime` 下間隔 1.28 秒）。參數由 `flat_stream` 改為 `windows`。

### ⚠️ 驗收基準變更
`tests/golden/main_acceptance.txt` 已更新。舊基準描述的是人工訊號，非真實資料：
...
| S01 品質合格率 | 3.5% | 5.5% |
| S10 品質合格率 | 20.6% | 22.5% |
| L7 欄位 | 預測次數（無從判讀）| 召回率（S08=93.3%、S10=66.7%）|

### Notes
- 模型準確率基準（S02=0.8430、S07=0.9045、S09=0.9065、S10=0.9029）**不受影響**——該測試直接使用預處理視窗，未經攤平路徑。
- 端到端驗證：`ui_bridge.py --realtime` 視窗推播中位數 1.279 秒（理論 1.280 秒，達成率 100.1%），抖動由 0.236 秒降至 0.064 秒。
- `RealTimeStreamProcessor` 的逐幀緩衝區不再被生產路徑使用，但仍由 `tests/test_stream_processor.py` 完整覆蓋，並保留為未來接入真實硬體的 API。
